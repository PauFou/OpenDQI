//! `DQI_FIELD_MISMATCH_RATE_SFTR` — share of SFTR per-trade
//! reconciliation records that carry at least one mismatched
//! matching-criterion (field-level discrepancy flagged by the
//! TR between the two counterparties' submissions).
//!
//! - **Layer:** auth.080 → [`crate::ReconciliationRecord`]
//!   filtered by `regime == Sftr`.
//! - **Denominator:** SFTR records with `reconciliation_status`
//!   populated (the field whose value triggers the
//!   `mismatched_fields` list).
//! - **Numerator:** SFTR records where `mismatched_fields` is
//!   non-empty.
//! - **Dimension:** consistency.
//!
//! Generic any-criterion-mismatch rollup. Per-criterion
//! breakdowns (notional / margin pre-haircut / margin post-
//! haircut / loan-value / collateral-value) are NOT separated
//! out in SFTR like they are in EMIR's auth.091 — the
//! per-criterion EMIR DQIs do not have SFTR mirrors in v0.17.

use crate::dq::dqi::compute::{resolve_threshold, EVIDENCE_TOP_N};
use crate::dq::dqi::{DqiEvidence, DqiIndicator, MappingPresence};
use crate::model::{DqDimension, ReconciliationRecord, Regime};
use crate::scoring::rate_with_status;
use crate::Thresholds;

const INDICATOR_ID: &str = "DQI_FIELD_MISMATCH_RATE_SFTR";
const DESCRIPTION: &str = "Share of SFTR reconciliation records with at least one mismatched \
matching-criterion (from auth.080 MsmtchFlds). Denominator restricted to records with \
reconciliation_status populated.";

/// Compute `DQI_FIELD_MISMATCH_RATE_SFTR`.
pub fn compute_dqi_field_mismatch_rate_sftr(
    recon_records: &[ReconciliationRecord],
    thresholds: &Thresholds,
    _mapping_presence: MappingPresence,
) -> (DqiIndicator, Vec<DqiEvidence>) {
    let mut denominator: u64 = 0;
    let mut numerator: u64 = 0;
    let mut offenders: Vec<DqiEvidence> = Vec::new();

    for r in recon_records {
        if r.regime != Regime::Sftr {
            continue;
        }
        if r.reconciliation_status.is_none() {
            continue;
        }
        denominator += 1;
        if r.mismatched_fields.is_empty() {
            continue;
        }
        numerator += 1;
        offenders.push(DqiEvidence {
            indicator_id: INDICATOR_ID.into(),
            uti: r.uti.clone().unwrap_or_default(),
            counterparty: r.reporting_counterparty.clone(),
            asset_class: None,
            source_file: r.source_file.clone(),
            observed_value: Some(r.mismatched_fields.join(",")),
            explanation: format!(
                "{} SFTR criterion(s) mismatched between counterparties",
                r.mismatched_fields.len()
            ),
        });
    }

    offenders.sort_by(|a, b| {
        a.source_file
            .cmp(&b.source_file)
            .then_with(|| a.counterparty.cmp(&b.counterparty))
            .then_with(|| a.uti.cmp(&b.uti))
    });
    offenders.truncate(EVIDENCE_TOP_N);

    let pair = resolve_threshold(thresholds, INDICATOR_ID);
    let (rate, status) = rate_with_status(numerator, denominator, &pair);
    let indicator = DqiIndicator {
        indicator_id: INDICATOR_ID.into(),
        regime: Regime::Sftr,
        dimension: DqDimension::Consistency,
        table_scope: "auth.080".into(),
        numerator,
        denominator,
        rate,
        threshold_amber: Some(pair.amber),
        threshold_red: Some(pair.red),
        status,
        description: DESCRIPTION.into(),
    };
    (indicator, offenders)
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::dq::dqi::DqiStatus;

    fn rec(
        uti: &str,
        regime: Regime,
        recon_status: Option<&str>,
        mismatched: Vec<&str>,
    ) -> ReconciliationRecord {
        ReconciliationRecord {
            uti: Some(uti.into()),
            regime,
            reconciliation_status: recon_status.map(|s| s.into()),
            mismatched_fields: mismatched.into_iter().map(String::from).collect(),
            ..Default::default()
        }
    }

    #[test]
    fn empty_input_is_not_applicable() {
        let (ind, _) = compute_dqi_field_mismatch_rate_sftr(
            &[],
            &Thresholds::default(),
            MappingPresence::default(),
        );
        assert_eq!(ind.status, DqiStatus::NotApplicable);
    }

    #[test]
    fn emir_records_ignored_by_regime_filter() {
        let recs = vec![rec("E1", Regime::Emir, Some("PAIRED"), vec!["NtnlAmt"])];
        let (ind, _) = compute_dqi_field_mismatch_rate_sftr(
            &recs,
            &Thresholds::default(),
            MappingPresence::default(),
        );
        assert_eq!(ind.denominator, 0);
    }

    #[test]
    fn no_recon_status_excluded() {
        let recs = vec![rec("S1", Regime::Sftr, None, vec!["NtnlAmt"])];
        let (ind, _) = compute_dqi_field_mismatch_rate_sftr(
            &recs,
            &Thresholds::default(),
            MappingPresence::default(),
        );
        assert_eq!(ind.denominator, 0);
    }

    #[test]
    fn all_clean_is_green() {
        let recs = vec![
            rec("S1", Regime::Sftr, Some("RECONCILED"), vec![]),
            rec("S2", Regime::Sftr, Some("RECONCILED"), vec![]),
        ];
        let (ind, _) = compute_dqi_field_mismatch_rate_sftr(
            &recs,
            &Thresholds::default(),
            MappingPresence::default(),
        );
        assert_eq!(ind.numerator, 0);
        assert_eq!(ind.status, DqiStatus::Green);
    }

    #[test]
    fn any_mismatch_counts() {
        let recs = vec![
            rec("S1", Regime::Sftr, Some("UNRECONCILED"), vec!["LoanVal"]),
            rec(
                "S2",
                Regime::Sftr,
                Some("UNRECONCILED"),
                vec!["CollVal", "Hrcut"],
            ),
            rec("S3", Regime::Sftr, Some("RECONCILED"), vec![]),
        ];
        let (ind, ev) = compute_dqi_field_mismatch_rate_sftr(
            &recs,
            &Thresholds::default(),
            MappingPresence::default(),
        );
        assert_eq!(ind.denominator, 3);
        assert_eq!(ind.numerator, 2);
        assert_eq!(ev.len(), 2);
    }
}
