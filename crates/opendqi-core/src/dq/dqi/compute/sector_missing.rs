//! `DQI_SECTOR_MISSING` — share of EmirRecord (TAR) records
//! where `corporate_sector` is missing or empty.
//!
//! - **Layer:** TAR.
//! - **Denominator:** all EmirRecord rows.
//! - **Numerator:** records with no `corporate_sector` value.
...
const INDICATOR_ID: &str = "DQI_SECTOR_MISSING";
const DESCRIPTION: &str = "Share of TAR records with no corporate_sector classifier.";

/// Compute `DQI_SECTOR_MISSING`.
pub fn compute_dqi_sector_missing(
    tar: &[EmirRecord],
    ...
    for r in tar {
        ...
        offenders.push(DqiEvidence {
            ...
            observed_value: None,
            explanation: "corporate_sector classifier missing".into(),
        });
    }
...
mod tests {
    ...
    fn rec(uti: &str, sector: Option<&str>) -> EmirRecord {
        EmirRecord {
            uti: Some(uti.into()),
            corporate_sector: sector.map(|s| s.into()),
            ..Default::default()
        }
    }
    ...
    fn empty_is_not_applicable() {
        let (ind, _) =
            compute_dqi_sector_missing(&[], &Thresholds::default(), MappingPresence::default());
        assert_eq!(ind.status, DqiStatus::NotApplicable);
    }

    #[test]
    fn all_present_is_green() {
        let recs = vec![rec("U1", Some("BANK")), rec("U2", Some("INSU"))];
        let (ind, _) =
            compute_dqi_sector_missing(&recs, &Thresholds::default(), MappingPresence::default());
