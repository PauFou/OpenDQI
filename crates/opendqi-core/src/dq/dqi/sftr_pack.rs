...
use crate::dq::{
    ...
    default_sftr_reu_state_checks, default_sftr_tr_state_checks, run_all_sftr, run_all_sftr_mar,
    run_all_sftr_msr, run_all_sftr_reu, run_all_sftr_reu_state, run_all_sftr_tr_state,
    CheckContext,
};
use crate::model::{
...
pub struct SftrDqiInputs<'a> {
    ...
    /// / ERRT / CORR / CRUD). Powers the 2 SFTR reuse DQIs
    /// (B4) and the 2 `SFTR.REU.*` granular checks (B5).
...
pub fn compute_sftr_dqi_pack(
    ...
    if let Some(reuse_state) = inputs.reuse_state {
        // v0.18 C5: fire the 2 SFTR.REU.STATE.* granular checks
        // on top of the 2 aggregate reuse-state DQIs from C4.
        files_processed += 1;
        records_processed = records_processed.saturating_add(reuse_state.len() as u32);
        all_issues.append(&mut run_all_sftr_reu_state(
            &default_sftr_reu_state_checks(),
            reuse_state,
            &ctx,
        ));
    }
...
mod tests {
    ...
    #[test]
    fn default_inputs_have_reuse_state_slot_at_none() {
        // v0.18 C3 contract: the reuse_state slot exists and
        // defaults to None alongside the other 7 slots. No DQI
        // computer reads it yet — that lands in C4 (2 SFTR
        // reuse-state DQIs) and C5 (2 SFTR.REU.STATE.* granular
        // checks).
        let inputs: SftrDqiInputs = SftrDqiInputs::default();
        assert!(inputs.reuse_state.is_none());
    }
