//! EMIR.MSR.INITIAL_MARGIN_NEGATIVE — current IM posted or collected
//! is negative.
...
pub struct EmirMsrInitialMarginNegative;

const CHECK_ID: &str = "EMIR.MSR.INITIAL_MARGIN_NEGATIVE";

impl MarginStateCheck for EmirMsrInitialMarginNegative {
...
mod tests {
    ...
    fn flags_negative() {
        ...
        let out = EmirMsrInitialMarginNegative.run(&[r], &[], &ctx());
        assert_eq!(out.len(), 1);
    }
    ...
    fn accepts_positive() {
        let r = MarginStateRecord {
            initial_margin_posted_current: Some(Decimal::from(1000)),
            ..Default::default()
        };
        let out = EmirMsrInitialMarginNegative.run(&[r], &[], &ctx());
        assert!(out.is_empty());
