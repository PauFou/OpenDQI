...
def main() -> None:
    ...
    nb.cells = [
        ...
        md(
            ...
            "(+ 16 SFTR in v0.17 — see Pattern 8 below).\n"
            "\n"
        ...
        md(
            "## Pattern 8 — SFTR Data Quality Pack (v0.17+, 16 indicators)\n"
            "\n"
            "Sister API on the SFTR side. v0.17 expanded from v0.16's 4 "
            "T2-layer-only DQIs to **16 SFTR indicators across 5 input "
            "layers** — the full SFTR mirror of the EMIR pack adapted to "
            "the real ESMA SFTR message set :\n"
            "\n"
            "| Layer | Count | Examples |\n"
            "|---|---|---|\n"
            "| TSR `auth.079` | 6 | LOAN/COLLATERAL_VALUE_*, HAIRCUT_ANOMALY, LEI_MISSING, UNDER_COLLATERALIZATION |\n"
            "| TAR `auth.052` | 1 | TIM_REPORTING_LATE |\n"
            "| Recon `auth.080` | 4 | PAIRING_RATE, RECONCILIATION_RATE, UNPAIRED_TRADES_RATE, FIELD_MISMATCH_RATE |\n"
            "| MCR `auth.083` | 1 | MCR_OPEN_REQUESTS |\n"
            "| MSR `auth.085` | 4 | T3_MARGIN_POSTED/RECEIVED_MISSING, T3_EXCESS_COLLATERAL_USE, T3_MARGIN_STALE |\n"
            "\n"
            "(Every indicator ID is suffixed `_SFTR`.) The MSR layer ALSO "
            "triggers 6 granular `SFTR.T3.*` per-record checks that flow "
            "into `result.issues` alongside the aggregated DQIs.\n"
            "\n"
            "**Paths-only in v0.17** (no pyarrow.Table dual-input on the "
            "SFTR side yet — v0.18). Same `PyDqiPackResult` shape as "
            "Pattern 7 — same 4 fields, same v1.0 Arrow schemas."
        ),
        code(
            ...
            "    reconciliation=str(sftr_kit / 'reconciliation.xml'),\n"
            "    missing_collateral=str(sftr_kit / 'missing_collateral.xml'),\n"
            "    msr=str(sftr_kit / 'msr.xml'),\n"
            "    as_of='2026-05-21',\n"
            ")\n"
            "print(repr(sftr_pack))\n"
            "\n"
            "# 16 SFTR indicators across 5 source layers\n"
            "sftr_df = sftr_pack.indicators.select(\n"
            "    ['indicator_id', 'table_scope', 'status', 'numerator', 'denominator', 'rate']\n"
            ").to_pandas()\n"
            ...
            "\n"
            "# Granular SFTR.T3.* checks (v0.17 F1') are co-produced into\n"
            "# result.issues when --msr is provided. Surface a few of them:\n"
            "import pyarrow.compute as pc\n"
            "t3_issues = sftr_pack.issues.filter(\n"
            "    pc.starts_with(sftr_pack.issues.column('check_id'), 'SFTR.T3.')\n"
            ")\n"
            "print(f'\\n=== SFTR.T3.* granular checks ({t3_issues.num_rows}) ===')\n"
            "if t3_issues.num_rows > 0:\n"
            "    print(t3_issues.select(['check_id', 'uti', 'message']).to_pandas().head(5).to_string(index=False))"
        ),
        md(
            "## Where to go next\n"
            "\n"
