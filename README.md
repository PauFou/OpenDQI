...
- **Data Quality Pack (v0.16)** : [`docs/data-quality-pack.md`](docs/data-quality-pack.md) — 28 regulator-style indicators (24 EMIR + 4 SFTR) above the 216 granular checks ; the "committee-readable" view. TARGET2 business-day calendar for stale-data indicators.
...
| Version | Theme |
|---|---|
| **v0.22.0** (current) | **Audit-driven polish** — closes 3 CRITICAL + 4 HIGH + 3 MEDIUM + 1 LOW findings from post-v0.21 audit. README install links bumped, `docs/data-quality-pack.md` count corrected, CODE_OF_CONDUCT + ISSUE_TEMPLATE + PULL_REQUEST_TEMPLATE added, branch protection on `main` enabled (5 required status checks), 6 edge-case tests for v0.20 parsers, PyPI version badge. **ZERO code logic change**, v1.0 Arrow contract unchanged. |
| **v0.21.0** | **Polish** — 5 SFTR Python wrappers (`opendqi.sftr.{mar,msr,reuse_activity,reuse_state,tr_status_advice}_scan`) reach parity with their CLI counterparts ; 7 byte-equality goldens for the v0.20 CLI subcommands ; `examples/{emir,sftr}/README.md` fixture indexes. SFTR Python entry points 9 → 14. |
| **v0.20.0** | **Perfect ESMA mirror + SFTR/EMIR parity (CLI + Python)** — EMIR coverage 8/11 → 10/10 actually-existing messages (auth.029 + auth.031 envelope-only parsers, auth.078 documented as non-existent in the ESMA bundle). 5 new SFTR standalone CLI subcommands (mar/msr/tr-status-advice/reuse-activity/reuse-state) + 2 new EMIR (query/status-advice). 7 new EMIR Python wrappers reaching parity with their CLI counterparts (`mar_scan`, `msr_scan`, `recon_stats`, `warnings`, `feedback`, `query_scan`, `status_advice_scan`). Granular checks 216 → 218 (+2 EMIR.{QRY,ACK}.ENVELOPE_WELLFORMED). |
